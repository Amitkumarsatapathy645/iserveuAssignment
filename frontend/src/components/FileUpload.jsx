...
const Alert = ({ children, variant = 'default' }) => {
  const bgColor = variant === 'success' ? 'bg-green-50 border-green-500' : 
                 variant === 'destructive' ? 'bg-red-50 border-red-500' : 
                 'bg-gray-50 border-gray-500';
  
  return (
    <div className={`p-4 rounded-lg border ${bgColor} mt-4`}>
      {children}
    </div>
  );
};
...
const FileUpload = () => {
  ...
  const validateFile = (file) => {
    ...
    if (!ALLOWED_FILE_TYPES.includes(file.type)) return 'Invalid file type. Please upload an Excel (.xlsx) file';
    return null;
  };
